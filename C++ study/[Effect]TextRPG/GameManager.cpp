...
{
	// 1. EffectManager 최우선 로드 (의존성 해결)
	effectManager.LoadFromCSV("TextRPG_Effect.csv");

	// 2. EffectManager를 참조하여 Item 및 Skill 로드
	itemManager.LoadFromCSV("TextRPG_sheet.csv", effectManager);
	itemManager.DisplayItemList();

	skillManager.LoadFromCSV("TextRPG_Skill.csv", effectManager);
